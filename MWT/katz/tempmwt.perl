...
#        $text1 =~ s/([^0-9])\.(\s+[A-Z])/$1\ ;$2/g;	# remove punctuation (period - nondec)
    ...
    print "text1[$cj]: $text1\n" if $debug;
    @texts[$cj] = $text1; 
    $cj++;
} #foreach (@rads)
$text = join("\n\n",@texts);

print "item: $text" if $debug;
...
system("sort terms.$date.$expr.outfile | uniq -c | sort -nr | temp_cutoff.perl >> terms.$date.$expr");
