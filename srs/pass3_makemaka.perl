...
my $pardagar    = DateTime::Duration->new( days => 2);
#==============================================================================================================================
open GIFT,"<mm"; #make_maka.txt";
while (<GIFT>) {
my @v = split;
$gift{$v[0]}{$v[1]}++;
$gift{$v[1]}{$v[0]}++;
...
# 120-135
print "F_makemaka_000_2y	F_makemaka_025_2y	F_makemaka_050_2y	F_makemaka_075_2y	F_makemaka_100_2y	F_makemaka_brutto_2y	F_makemaka_netto_2y	F_makemaka_summa_utbetalt_2y	";
print "F_makemaka_000_hittills	F_makemaka_025_hittills	F_makemaka_050_hittills	F_makemaka_075_hittills	F_makemaka_100_hittills	F_makemaka_brutto_hittills	F_makemaka_netto_hittills	F_makemaka_summa_utbetalt_hittills	";
# 136-171
print "M_makemaka_000_2y	M_makemaka_025_2y	M_makemaka_050_2y	M_makemaka_075_2y	M_makemaka_100_2y	M_makemaka_brutto_2y	M_makemaka_netto_2y	M_makemaka_summa_utbetalt_2y	";
print "M_makemaka_000_hittills	M_makemaka_025_hittills	M_makemaka_050_hittills	M_makemaka_075_hittills	M_makemaka_100_hittills	M_makemaka_brutto_hittills	M_makemaka_netto_hittills	M_makemaka_summa_utbetalt_hittills	";
print "\n";
#==============================================================================================================================
while (<>) {
    chop;
    ...
    next unless $lopnr > 0;
    print;
    $item{$lopnr}{$start{$index}} = @sjukskrivningsfall[2 .. 10]; # slutdatum + den grejen
    $diagnos{$lopnr}{$start{$index}} = $sjukskrivningsfall[77];
    @{ $sammadiagnos{$lopnr}{$start{$index}} } = @sjukskrivningsfall[27 .. 42];
#filen ska vara sorterad, så dessa sparar vi bara den senaste noteringen på!
    @{ $alladiagnos{$lopnr} } = @sjukskrivningsfall[11 .. 26];
    @{ $Fdiagnos{$lopnr} } = @sjukskrivningsfall[43 .. 58];
    @{ $Mdiagnos{$lopnr} }  = @sjukskrivningsfall[59 .. 74];
    $makemaka = $sjukskrivningsfall[102];
    unless ($makemaka) {print "\n"; next;} else {print "\t";}
# kolla om maken är sjuk just då? if (sjuk nyss dvs parser tidpunkt period > något)  print yes print for $item else print for item 0
    print "vetej\t";
# kolla om maken sjuk vid någon period strax innan?
# kanske kolla om hemma med barn också?
# kolla om maken någon gång i sin historia haft samma diagnos?
    for ($a=0;$a<16;$a++) {print $alladiagnos{$makemaka}[$a]+0;print "\t";}
    for ($a=0;$a<16;$a++) {print $Fdiagnos{$makemaka}[$a]+0;print "\t";}
    for ($a=0;$a<16;$a++) {print $Mdiagnos{$makemaka}[$a]+0;print "\t";}
    print "\n";
}
