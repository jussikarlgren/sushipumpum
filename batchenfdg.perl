...
$target_dir = "/hosts/krumelur/e2/clef/$lang/LA-fdg";
#-------------------------------------
opendir(DATEDIR,$dir);
...
foreach $filename (@DATAFILES) {
	...
	if (m-^</HEADLINE- || m-^\</TEXT-) {
	    ...
	    close OUTFILE;
	    $text =~ s/\<[^\>]*\>/\n/g ;	# remove tags
	    open(OUTPIPE,"| /usr/local/conexor/en/fdg/$lang-fdg 2>> batchenfdg.log >> $target_dir/$year/$month/$year$month$day.fdg.xml"); 
	    print OUTPIPE $text; 
	    close(OUTPIPE);	
	    $text = "";
	    open(OUTFILE,">>$target_dir/$year/$month/$year$month$day.fdg.xml"); 
